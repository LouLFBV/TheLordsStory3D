...
public class BossAI3 : EnemyParent
{
    [Header("Boss Stats")]
    ...
    public void ActiveIsAttacking() {isAttacking = true; }
    public void DesactiveIsAttacking() { isAttacking = false;}
    ...
    public void SpawnFireBall(int attackNumber)
    {
        BossAttack attack = attacksBoss[attackNumber-1];
        if (attack.fireBallPrefab != null && attack.firePoint != null)
        {
            GameObject fireBall = Instantiate(attack.fireBallPrefab, attack.firePoint.position, attack.firePoint.rotation);
            if (fireBall != null)
            {
                HitBoxAttack hitBoxAttack = fireBall.GetComponent<HitBoxAttack>();
                if (hitBoxAttack != null)
                {
                    hitBoxAttack.damageAmount = attack.damage;
                    hitBoxAttack.damageType = attack.damageType;
                    hitBoxAttack.isFireBreath = attack.isFireBreath;
                }
            }
            ...
            {
                rb.useGravity = false;
                rb.linearVelocity = attack.firePoint.forward * attack.fireBallSpeed;
            }
            Destroy(fireBall, 5f);
        }
    }
